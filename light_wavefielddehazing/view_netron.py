...
用法:
  ...
  python view_netron.py --theme dark        # 深色主题（若 Netron 支持）
...
def main():
    ...
    parser.add_argument("--theme", choices=["light", "dark", "system"], default="system",
                        help="显示主题: light 浅色, dark 深色, system 跟随系统")
    ...
    if not args.no_browser:
        try:
            import netron
            start_kw = {"port": args.port}
            if args.theme in ("dark", "light"):
                start_kw["theme"] = args.theme
            try:
                netron.start(out_path, **start_kw)
            except TypeError:
                netron.start(out_path, port=args.port)
            print("Netron 已在浏览器中打开，端口:", args.port)
            if args.theme != "system":
                print("  主题:", "深色" if args.theme == "dark" else "浅色")
                print("  若未生效: 在 Netron 界面点击 View → 勾选 Dark mode 切换主题")
            else:
                print("  切换样式: 使用 --theme dark 或 --theme light；或在 Netron 中 View → Dark mode")
        except ImportError:
