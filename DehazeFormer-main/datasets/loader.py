...
class PairLoader(Dataset):
	def __init__(self, data_dir, sub_dir, mode, size=256, edge_decay=0, only_h_flip=False, data_ratio=1.0):
		assert mode in ['train', 'valid', 'test']
		assert 0 < data_ratio <= 1.0, 'data_ratio 应在 (0, 1]'
		...
		all_names = sorted(os.listdir(os.path.join(self.root_dir, 'GT')))
		n_use = max(1, int(len(all_names) * data_ratio))
		self.img_names = all_names[:n_use]
