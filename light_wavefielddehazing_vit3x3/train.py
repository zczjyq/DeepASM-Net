...
def main():
    ...
    for epoch in range(start_epoch, cfg["train"]["epochs"] + 1):
        ...
        for step, (hazy, clean, _, _) in enumerate(pbar):
            ...
            with torch.no_grad():
                metric_pred = pred.detach().float()
                metric_clean = clean.detach().float()
                batch_psnr = psnr(metric_pred, metric_clean).item()
                batch_ssim = ssim(metric_pred, metric_clean).item()
            ...
            pbar.set_postfix(
                ...
                psnr=f"{running_psnr / n_steps:.2f}",
                ssim=f"{running_ssim / n_steps:.4f}",
            )

        train_loss = running_loss / max(1, n_steps)
        train_psnr = running_psnr / max(1, n_steps)
        train_ssim = running_ssim / max(1, n_steps)
        ...
        print(
            f"Epoch {epoch}: "
            f"train_loss={train_loss:.6f}, train_psnr={train_psnr:.4f}, train_ssim={train_ssim:.4f}, "
